...
class RombelController extends Controller
{
    ...
    {
        // Ambil semua tahun akademik aktif (misalnya hanya 1 aktif)
        $tahunList = TahunAkademik::where('semester_aktif', 'aktif')
            ->orderBy('tahun_akademik', 'desc')
            ->get();

        // Jika ada permintaan AJAX berdasarkan tahun
        if ($request->has('tahun')) {
            $data = Detail::getPembagianKelas($request->tahun);
            return response()->json($data);
        }

        // Ambil tahun akademik aktif pertama untuk filter santri
        $tahunAktif = $tahunList->first(); // bisa null kalau tidak ada

        // Cek jika ada tahun aktif
        if ($tahunAktif) {
        ...
        return view('rombel.index', compact('tahunList', 'sekolah', 'santri', 'kelas'));
    }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($request->santri_id as $santriId) {
            Detail::create([
                'tahun_akademik_id' => $tahunAktif->id_tahun_akademik,
    ...
    public function hapusSantri(Request $request)
    {
        ...
        foreach ($request->santri_id as $nis) {
            Detail::where('id_detail', $request->detail_id)
                ->where('santri_id', $nis)
                ->delete();
        }
    ...
    public function naikKelas(Request $request)
    {
        ...
        $tahunBaru = TahunAkademik::where('semester_aktif', 'aktif')->orderByDesc('id_tahun_akademik')->first();

        foreach ($request->santri_id as $nis) {
            Detail::create([
                'kelas_id' => $request->kelas_tujuan_id,
                'tahun_akademik_id' => $tahunBaru->id_tahun_akademik,
                'santri_id' => $nis,
            ]);
        }

        return redirect()->back()->with('success', 'Siswa berhasil dinaikkan ke kelas selanjutnya.');
    }
}
